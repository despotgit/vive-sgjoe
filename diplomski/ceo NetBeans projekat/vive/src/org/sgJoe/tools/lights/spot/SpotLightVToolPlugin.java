...
/*
 * Descritpion for SpotLightVToolPlugin.java
 *
 *
 * @author   $ Author: Aleksandar Babic  $
 * @version  $ Revision:             0.1 $
 * @date     $ Date: June 4, 2006  6:37 PM  $
 */

public class SpotLightVToolPlugin extends LightVToolPlugin {
    
    private static Logger logger = Logger.getLogger(SpotLightVToolPlugin.class);
   
    public SpotLightVToolPlugin(VirTool virToolRef) {
        super(virToolRef);
    }
   ...
   public void performAction(Form form, SceneGraphEditor editor, Session session) throws SGPluginException {
        ...
        if(action == VToolForm.ACT_MOUSE_CLICKED) {
            ...
            SpotLightVToolForm vtForm = (SpotLightVToolForm) toolForm;
        ...
        } else if(action == VToolForm.ACT_LIGHT_ATTENUATION_CHANGE) {
            Light light = vTool.getLight();
            SpotLightVToolForm ptForm = (SpotLightVToolForm) toolForm;
            Point3f attenuation = new Point3f(ptForm.getAttC(), ptForm.getAttL(), ptForm.getAttQ());
            
            ((SpotLight)light).setAttenuation(attenuation);

        } else if(action == VToolForm.ACT_LIGHT_CONCENTRATION_CHANGE) {
            SpotLightVToolForm ptForm = (SpotLightVToolForm) toolForm;
            Double concentration = new Double(ptForm.getConcentration());
            ((SpotLight)vTool.getLight()).setConcentration(concentration.floatValue());

        } else if(action == VToolForm.ACT_LIGHT_SPREAD_ANGLE_CHANGE) {
            SpotLightVToolForm ptForm = (SpotLightVToolForm) toolForm;
            Double spreadAngle = new Double(Math.toRadians(ptForm.getSpreadAngle()));
            ((SpotLight)vTool.getLight()).setSpreadAngle(spreadAngle.floatValue());
        } else {
